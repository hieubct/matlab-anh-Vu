...
M2 = 0.57;%1-(cbtt2/cl);
M3 =0.57;% 1-(cbtt3/cl);
anpha1 = 0;%0.009*pi/180;% goc nghieng cua pad1
anpha2 = 0.0175*pi/180;%0.025*pi/180;%0.0196*pi/180;% 0.02*pi/180;%0.0189*pi/180;% goc nghieng cua pad2
anpha3 = 0;% goc nghieng cua pad3 
%Precess1=0;% áp su?t vùng lõm2
Precess2=1.45;%1.45;%1.4;%7.5;%3.43;% áp su?t vùng lõm2
...
exilon = 0.1;%0.25;%0.05;%e1/c1; % ti le lech tam
phibd = 1.12;%0.7865; % goc lech ban dau cua truc
        ...
        for i =1:m+1
            for j =1:n+1
                if j==1||j==n+1||i==1||i==m+1
                    p2s(i,j)=0;
                    %p(i,j)=0;%p(i,j+1);
                elseif i>=42&&i<=120&&j>=15&&j<=77%i>=46&&i<=116&&j>=19&&j<=73
                   p2s(i,j)=Precess2;
                else
                    a2s(i,j)= hc2s(i,j)^3;
                    b2s(i,j)= ht2s(i,j)^3;
...
Z3=p3;
mesh(X3,Y3,Z3)
xlabel('dimensionless pressure');
ylabel('dimensionless pressure');
zlabel('dimensionless pressure');
hold off
colorbar
