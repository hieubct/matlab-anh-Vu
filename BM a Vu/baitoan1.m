function baitoan
   ...
   % mi=[];
    ...
    xic3
    %plot(xicma,mi)
    fxPfy(phibd,xic1,0,120);
    fxPfy(phibd,xic2,120,240);
    fxPfy(phibd,xic3,240,360);
    Fx/Fy
end
...
function fxPfy(phibd,xicma,alfa,beta)
        %global mi1 e c cb nguy l R d posi M ld m n exilon coxi xic1
        global Fx Fy
        mi1=0;
        coxi = 0.001;
    ...
    ld = l/d;   
        ...
        for i= 1:m+1
           for j=1:n+1
               Fx=Fx+P(i,j)*cos(phi11+phi(i,j))*deltaphi1*deltalanda;% luc theo truc x
               Fy=Fy+P(i,j)*sin(phi12+phi(i,j))*deltaphi1*deltalanda;% luc theo truc y
           end
        end
